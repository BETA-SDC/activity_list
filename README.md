...
该网站会自动读取 `data/activities`、`data/calendar` 和 `data/2026` 文件夹下的分组数据，向内部成员展示项目计划与分工。
网站包含三个页面：
...
- `data/2026` 文件夹下的分工文件，优先读取 `data/2026/index.json`
...
建议先在 `data/2026/index.json` 中列出需要加载的文件，再配合连续编号管理：
...
`data/activities` 中的“代号”必须和 `data/2026` 文件中的“代号”一致。例如活动是 `P2`，分工文件里也要写 `"代号": "P2"`，否则“已分工”状态无法自动匹配分工明细。

### 2. 用清单文件管理分工文件

如果存在 `data/2026/index.json`，网站会优先按清单读取，适合长期维护和不连续编号的情况。
...
如果没有清单文件，网站仍会从 `data/2026/2026-P1.json` 开始自动读取，连续两次找不到文件时停止。
