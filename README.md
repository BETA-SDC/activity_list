...
- `活动表.json`
- `任务分工` 文件夹下的分工文件，优先读取 `任务分工/index.json`
...
建议先在 `任务分工/index.json` 中列出需要加载的文件，再配合连续编号管理：
...
如果存在 `任务分工/index.json`，网站会优先按清单读取，适合长期维护和不连续编号的情况。
...
如果没有清单文件，网站仍会从 `任务分工-P1.json` 开始自动读取，连续两次找不到文件时停止。
...
1. 新活动优先追加到 `活动表.json`，再同步补分工文件和清单。
